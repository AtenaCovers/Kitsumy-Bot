...
—◉ *Contacto para apelaciones:* wa.me/5493838525316
╚═════════════════════════╝
               `.trim();
              m.reply(messageText);
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;David Delgado 👑;;;\nFN:David Delgado 👑\nORG:David Delgado 👑\nTITLE:\nitem1.TEL;waid=5219992095479:+521 999 209 5479\nitem1.X-ABLabel:David Delgado 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:David Delgado 👑\nEND:VCARD`;
        await this.sendMessage(nk.from, {contacts: {displayName: 'David Delgado 👑', contacts: [{vcard}]}}, {quoted: callmsg});
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '[ ⚠ ] 𝐀𝐕𝐈𝐒𝐎 - 𝐀𝐋𝐄𝐑𝐓𝐀', body: 'Kitsumy-Bot, thumbnail: imagen1, sourceUrl: 'https://github.com/AtenaCovers/Kitsumy-Bot'}}}}, aa);
